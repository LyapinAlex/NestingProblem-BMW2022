...
class Polygon:
    ...
    def __repr__(self):
        return 'Polygon(n = {})'.format(self.num_sides)
    ...
    def create_rastr_approximation(self, h: float) -> np.ndarray:
    ...
    def point_reflection(self):
        """Центрально симметричное отображение многоугольника"""
        for point in self.points:
            point *= -1
        return self

    def axial_symmetry(self):
        """Отображение многоугольника относительно прямой x=y"""
        for point in self.points:
            point.axial_symmetry()
        return self

    def axial_symmetry_y(self):
        """Отображение многоугольника относительно оси Ox"""
        for point in self.points:
            point.x *= -1
        return self
    ...
    def draw(self,
             ...
             h=None):
        """Под остальным имеется в виду барицентр и расширение"""
        fig, ax = plt.subplots()
        pr.image_size(fig, self.size)
        pr.focusing_on_subject(ax, self.minXY(), self.maxXY())

        if is_draw_raster_approximation:
            matrix = self.create_rastr_approximation(h)
            pr.draw_raster_approximation(ax, self, matrix, h)

        if is_draw_other:
            pr.draw_polygon(ax,
                            self.expand_polygon(indent_expand_polygon),
                            edgecolor='green')
            bar = self.calc_centroid()
            plt.plot(bar.x, bar.y, 'co')

        pr.draw_polygon(ax, self)
        plt.show()
